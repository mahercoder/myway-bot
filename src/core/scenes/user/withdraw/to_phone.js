...
const callback_data = {
    ...
    back: 'user.withdraw.withdraw_to_phone.back'
}
...
const scene = new BaseScene('user-home-withdraw-to_phone');

scene.enter( async ctx => {
    const user = await Users.findOne({ where: { id: ctx.from.id }})
    const caption = ctx.i18n.t('user.withdraw.withdraw_to_phone.caption', { balance: user.balance })
    ...
    if (!isNaN(withdrawalCost)) {
        if(config.getMinimalWithdraw() <= withdrawalCost && withdrawalCost > 0){
            ...
            if(user.balance >= withdrawalCost){
                try {
                    const paymentReceivedText = ctx.i18n.t('user.withdraw.withdraw_to_phone.payment_received', { withdrawalCost })
                    
                    const cardOrPhone = ctx.session.phoneNumber
                    const paymentMethod = ctx.i18n.t('user.withdraw.method.phone')
                ...
                const notEnoughMoneyText = ctx.i18n.t('user.withdraw.withdraw_to_phone.not_enough_money')
                await ctx.replyWithHTML(notEnoughMoneyText)
            }
        } else {
            const errorMinimalWithdrawText = ctx.i18n.t('user.withdraw.withdraw_to_phone.error_minimal_withdraw', { minimalCost: config.getMinimalWithdraw() })
        ...
        const errorText = ctx.i18n.t('user.withdraw.withdraw_to_phone.error_cost')
